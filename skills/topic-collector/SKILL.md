...
description: AI热点采集工具。从Twitter/X、Product Hunt、Reddit、Hacker News等采集AI相关热点，通过飞书消息推送给自己。聚焦领域：Vibe Coding/AI编程、AI新产品/创业、AI模型动态。当用户说"开始今日选题"、"采集热点"、"看看今天有什么新闻"、"今日AI热点"、"帮我采集"时触发。
icon: 📰
---

# Topic Collector - AI热点采集 → 飞书推送
...
使用 **lark-im** skill 将整理好的内容发送给自己（账号：taojiaxuan111@gmail.com）。
...
| "今日AI热点" | 完整采集流程，飞书发送 |
| "帮我采集" | 完整采集流程，飞书发送 |
| "只看 Vibe Coding 热点" | 只搜索 Vibe Coding 分类，飞书发送 |
| "只看模型动态" | 只搜索模型动态分类，飞书发送 |
...
- **时间严格限制 24 小时**，不采集旧内容
- 每次推送前告诉用户"正在搜索 X 分类..."让用户知道进度
- 如果某分类搜索结果为空，跳过该分类，不要填充旧内容
- 链接必须是原文直链，不要用重定向或聚合页链接
- 飞书消息发送完成后，告诉用户"已发送到你的飞书"
